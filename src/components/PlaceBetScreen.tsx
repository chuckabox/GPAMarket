...
const PlaceBetScreen: React.FC<PlaceBetScreenProps> = ({ onBack }) => {
  ...
  const platformFeeRate = 0.20; // 5%

  // Restructured logic for "Plateau then Jump" scaling
  const calculateStats = (target: string) => {
    const targetNum = parseInt(target);
    const distance = Math.abs(currentGPA - targetNum);
    
    let multiplier = 1.05; // Base starting point
    let probability = 85.0;

    if (distance <= 1.5) {
      // Safe zone (1.xx)
      multiplier += distance * 0.45; 
      probability -= distance * 15;
    } else if (distance <= 2.2) {
      // Mid zone (2.xx)
      multiplier = 1.8 + (distance - 1.5) * 1.5;
      probability = 40 - (distance * 10);
    } else {
      // High risk zone (Capped at 5.00)
      multiplier = 3.0 + Math.pow(distance - 2.2, 1.5) * 1.2;
      probability = Math.max(1.2, 15 / (distance - 1));
    }
  ...
  const potentialPayout = (numericWager * parseFloat(multiplier)).toFixed(2);
  const feeAmount = (numericWager * platformFeeRate).toFixed(2);
        ...
        <div className="p-6 bg-white dark:bg-dark-surface border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-[13px] text-slate-500 dark:text-dark-muted font-medium mb-2">Introduction to Software Engineering (CSSE1001)</p>
        ...
        <div className="px-6 py-8 flex flex-col gap-8">
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-slate-400 dark:text-dark-muted">Volatile odds for unlikely outcomes for final exam</span>
            </div>
          ...
          <section className="bg-brand-blue/5 dark:bg-brand-blue/10 border border-brand-blue/10 dark:border-brand-blue/20 rounded-xl p-5">
            <div className="flex flex-col gap-3">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className={`font-semibold ${parseFloat(probability) < 10 ? 'text-red-500' : 'text-slate-700 dark:text-dark-text'}`}>
                  {probability}%
                </span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-500 dark:text-dark-muted">Platform Fee (5%)</span>
                <span className="font-semibold text-slate-700 dark:text-dark-text">-${feeAmount}</span>
              </div>
